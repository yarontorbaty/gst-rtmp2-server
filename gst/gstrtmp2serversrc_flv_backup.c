    ...
    if (src->active_client && 
      ...
      if (flv_tags) {
        ...
        if (tag->data && gst_buffer_get_size (tag->data) > 0) {
          ...
          /* FLV tag data = codec_byte + payload, but tag->data only has payload */
          gsize flv_data_size = 1 + payload_size;  /* +1 for codec byte */
          gsize total_size = 11 + flv_data_size + 4;  /* header + data + prev_tag_size */
          static gint frame_num = 0;
          frame_num++;
          
          /* Serialize FLV tag: [header(11)] + [codec_byte(1)] + [payload(N)] + [prev_tag_size(4)] */
          *buf = gst_buffer_new_allocate (NULL, total_size, NULL);
          ...
          if (gst_buffer_map (*buf, &map_out, GST_MAP_WRITE) &&
            ...
            *ptr++ = (flv_data_size >> 16) & 0xFF;              /* Data size (24-bit BE) */
            *ptr++ = (flv_data_size >> 8) & 0xFF;
            *ptr++ = flv_data_size & 0xFF;
            ...
            *ptr++ = 0; *ptr++ = 0; *ptr++ = 0;                 /* Stream ID (always 0) */
            ...
            /* Reconstruct codec byte from tag metadata */
            if (tag->tag_type == RTMP2_FLV_TAG_VIDEO) {
              guint8 frame_type = tag->video_keyframe ? 1 : 2;  /* 1=keyframe, 2=inter */
              guint8 codec_id = tag->video_codec & 0x0F;
              *ptr++ = (frame_type << 4) | codec_id;
            } else if (tag->tag_type == RTMP2_FLV_TAG_AUDIO) {
              guint8 codec = (tag->audio_codec & 0x0F) << 4;
              guint8 rate = (tag->audio_sample_rate & 0x03) << 2;
              guint8 size = (tag->audio_sample_size & 0x01) << 1;
              guint8 channels = tag->audio_channels & 0x01;
              *ptr++ = codec | rate | size | channels;
            } else {
              *ptr++ = 0;  /* Unknown tag type - placeholder */
            }
            
            /* Write tag payload (without codec byte) */
            memcpy (ptr, map_in.data, payload_size);
            ptr += payload_size;
            
            /* Write previous tag size (header + data = 11 + flv_data_size) */
            guint32 prev_tag_size = 11 + flv_data_size;
            ...
            *ptr++ = prev_tag_size & 0xFF;
            ...
            GST_INFO_OBJECT (src, "📤 FLV TAG #%d: type=%s data=%zu total=%zu ts=%u",
                frame_num,
                tag->tag_type == RTMP2_FLV_TAG_VIDEO ? "video" : "audio",
                flv_data_size, total_size, tag->timestamp);
            ...
            /* Failed to map - cleanup and error */
            if (map_out.data)
              gst_buffer_unmap (*buf, &map_out);
            if (map_in.data)
              gst_buffer_unmap (tag->data, &map_in);
